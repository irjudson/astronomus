...
def test_add_visibility_info_at_horizon():
    """Test that targets exactly at the horizon (altitude = 0) are marked as below_horizon."""
    # Mock database session
    db = MagicMock()
    service = CatalogService(db)

    # Mock ephemeris service - target is exactly at horizon
    ephemeris = MagicMock(spec=EphemerisService)
