    ...
    <div class="bg-gray-900 border border-gray-800 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
      ...
      <div v-else-if="activeTab === 'scope'" class="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <section>
          ...
          <!-- Step indicator (shown once wizard starts) -->
          ...
          <div class="bg-gray-800 rounded-lg p-4">
            ...
            <template v-else-if="polarStep === 1">
              ...
              <div class="flex items-start gap-4">
                ...
                <svg viewBox="0 0 100 100" class="w-24 h-24 flex-shrink-0">
                  ...
                  <!-- North target arrow (green, fixed pointing up) -->
                  ...
                  <!-- Current heading arrow (orange, live) -->
                ...
                <div class="flex-1 space-y-2 pt-1">
                  ...
                  <p class="text-xs text-gray-500 leading-tight">
                    Green = North target<br>
                    Orange dashed = current heading
                  </p>
            ...
            <!-- Step 2: Tilt to latitude elevation (live balance polling) -->
            <template v-else-if="polarStep === 2">
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1 space-y-2 pt-1">
                  ...
                  <p class="text-xs text-gray-500 leading-tight">
                    Green dashed = target {{ elevationTarget.toFixed(1) }}°<br>
                    Blue solid = current angle
                  </p>
                </div>
              </div>
              <div class="flex gap-2 mt-3">
...
// ── Polar alignment wizard ─────────────────────────────────────────────────────
const polarStep = ref(0)  // 0=idle, 1=compass, 2=elevation, 3=measuring, 4=results
let polarTimer = null
...
// Auto-advance from measuring to results when firmware completes
...
const handlePolarDone = () => {
  stopPolarPoll()
  stopPolarStepTimers()
  polarStep.value = 0
}

onUnmounted(() => { stopPolarElapsed(); stopPolarStepTimers() })
...
// ── Compass wizard step (Step 1) ───────────────────────────────────────────────
// SVG: 100x100, center (50,50), heading arrow radius 30px
const headingX2 = computed(() => {
  ...
  return (50 + 30 * Math.sin(h)).toFixed(1)
})

const headingY2 = computed(() => {
  if (executionStore.compass.heading === null) return 50
  const h = executionStore.compass.heading * Math.PI / 180
  return (50 - 30 * Math.cos(h)).toFixed(1)
})
...
const headingArrow2 = computed(() => {
  ...
  const w1x = (bx + 4 * cos).toFixed(1), w1y = (by + 4 * sin).toFixed(1)
  const w2x = (bx - 4 * cos).toFixed(1), w2y = (by - 4 * sin).toFixed(1)
...
const compassStatusText = computed(() => {
  ...
  return `Rotate ${dir} ${Math.round(delta)}° to face North`
})
...
const compassStatusClass = computed(() => {
  ...
  const h = executionStore.compass.heading
  const delta = h <= 180 ? h : 360 - h
  return delta < 45 ? 'text-yellow-400' : 'text-orange-400'
})

// ── Elevation wizard step (Step 2) ─────────────────────────────────────────────
// SVG: 105x80, origin (10, 65), line length 60px
const elevationTarget = computed(() => Math.abs(localSettings.value.latitude || 0))
...
const elevCurrentArcPath = computed(() => {
  const r = 22
  const ax = (10 + r * Math.cos(elevCurrentRad.value)).toFixed(1)
  const ay = (65 - r * Math.sin(elevCurrentRad.value)).toFixed(1)
  return `M ${10 + r},65 A ${r},${r} 0 0,0 ${ax},${ay}`
})

const elevCurrentLabelX = computed(() => {
  const mid = elevCurrentRad.value / 2
  return (10 + 32 * Math.cos(mid)).toFixed(1)
})

const elevCurrentLabelY = computed(() => {
  const mid = elevCurrentRad.value / 2
  return (65 - 32 * Math.sin(mid) + 4).toFixed(1)
})
...
// ── Setup guide SVGs (compass idle-state status indicators) ───────────────────
